...
function gt(t,n) { return (n||document).getElementsByTagName(t)[0]}
function ge(t,n) { return (n||document).getElementById(t)}
...
function add_area_to_header(){
    ...
    seting_area.innerHTML="Video URL Parse Site</br><select id='video_url_parse_site'><option value=1>twsaver.com</option><option value=2>twdown.net</option><option value=3>savetweetvid.com</option></select></br>"+
                          ...
                          "FileName Format</br><input id='filename_fmt' name='filename_fmt' type='text' value='%a - %c %t'>";
    ...
    document.getElementById('video_url_parse_site').value='3';
}

function add_button(p,t,fun,id){
    var b=document.createElement("button");
    b.innerHTML="<p style='margin: 2px; font-size: 13px;' >"+t+"</p>";
    b.addEventListener('mousedown', fun);
    b.oncontextmenu = function(e){e.preventDefault()}
    b.id=id;
    b.style="color: white;	background-color: #1da1f2;	border: 0px white;	border-radius: 5px;margin: 2px;height: 32px;width: 100px";
    if(p)p.appendChild(b);
    return b;
}
...
function download_all_init(){
     if(ge("auto_scroll").checked){
         if(download_all_scroll_to_end==false){
             download_all_scroll_to_end=true;
             ge("button_scroll_to_end").click();
             return;
         }
     }
    //init download param
    download_image=ge("image_include").checked;
    download_video=ge("video_include").checked;
    ...
    video_quality=Number(ge("video_qty").value);

    filename_fmt=ge("filename_fmt").value;
    if(filename_fmt==="")filename_fmt="%a - %c";//in case of empty filename_fmt;

    var first_article=get_by_xpath("//section/div/div/div/div/div/article",document);
    var continue_article=ge("current_target");
...
function query_article(){
        ...
        if(article){
            ...
            //    var expend=get_by_xpath("div/div/div/div[2]/div[2]/div[2]/div[2]/div/div/div/div[2]/article/div/div/div/div/div/div[2]/div/div[2]/div/div/span/span",article);
...
function download_filename_preprocess(article){
    unsafeWindow.article = article;
    let article_time=gt('time',article).getAttribute('datetime');
    ///html/body/div[1]/div/div/div[2]/main/div/div/div/div/div/div[2]/div/div/section/div/div/div[4]/div/div/article/div/div/div/div[2]/div[2]/div[2]/div[1]/div
    //let article_twitter_name_area=get_by_xpath("div/div/div/div[2]/div[2]/div[1]/div/div/div[1]/div[1]/a/div",article);
    let article_twitter_name_area=get_by_xpath("div/div/div/div[2]/div[2]/div[2]/div[1]/div",article);
    var article_twitter_name="";
    article_twitter_name_area?.querySelectorAll("span")?.forEach(function (x){ article_twitter_name+=x.innerText;});
    ...
    let temp=filename_fmt;
    temp=temp.replaceAll("%a",article_twitter_name.slice(0, 31));
...
function download_filename_postprocess(filename_preprocess,original_filename,qty_char=""){
    ...
    //console.log("download_filename_postprocess::", temp);
    temp=temp.replaceAll("%f",original_filename);
...
function download_article_content(article){
    ...
    if(download_image) download_article_image(article,filename_preprocess[0].replaceAll("%t",""));
    if(download_video) {
        var filename = filename_preprocess[0].replaceAll("%t",filename_preprocess[1]);
        download_article_video(article,filename);
    }
    query_next_article();
}

function download_file(url,filename,is_video){
    download_count++;
    //111
    ...
    if(is_video && !filename.endsWith('.mp4')) {
        filename+='.mp4';
    }
    var new_url = url.replaceAll(/\?.*/g, "");
    console.log("download_file::", filename, url);
    if(is_video) {
        const event = new CustomEvent('dirdwn', { detail: {url:url,name:filename}});
        window.dispatchEvent(event);
    } else {
        // temp =
        GM_download({
            ...
            name:filename,
            saveAs:false,
            onload:function(e){download_count--;console.log("");},
            onerror:function(e) {
                download_count--;
                console.error(e);
                if(new_url!=url) {
                    download_file(new_url,filename);
                }
            },
...
function download_article_video_twsaver_cb(result){
    ...
    console.log("let filename", filename, url, url.match("[^/]*?(?=\\?)"));

    download_file(url,filename,1);
    //Net_download(url,filename+".mp4");
   //GM_download({
   //        url:url,
   //        name:filename,
   //        saveAs:false,
...
function query_next_article(){
    ...
    if(is_download_all) {
        console.log("延时1秒...");
        setTimeout(function(){query_article();},1380);
    } else {
        status_message(null);
    }
}
