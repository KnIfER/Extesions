...
function download_filename_preprocess(article){
    let article_time=get_by_xpath("div/div/div/div[2]/div[2]/div[1]/div/div/div[1]/a/time/@datetime",article).value;
...
function download_article_content(article){
    ...
    if(download_video) download_article_video(article,filename_preprocess[0].replaceAll("%t",filename_preprocess[1]));
    query_next_article();
}
...
function download_article_video_twsaver_cb(result){
    ...
    //console.log("let filename", url, url.match("[^/]*?(?=\\?)"));
...
function query_next_article(){
    ...
    if(is_download_all){
        console.log("延时1秒...");
        setTimeout(function(){query_article();},218);
    }else{
        status_message(null);
    }
}
